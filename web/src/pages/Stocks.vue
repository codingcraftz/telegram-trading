...
function startPolling() {
  ...
  pollTimer = setInterval(loadQuotes, 2_500);
}
...
async function add(item: SearchItem) {
  try {
    ...
    if (r.existed) toast.info('이미 관심 종목에 담겨 있어요');
...
<template>
  <div class="space-y-4">
    ...
    <SymbolSearch placeholder="종목 이름 또는 6자리 코드" @pick="add" />
    ...
    <section class="space-y-2">
      ...
      <div v-if="data && data.items.length > 0" class="space-y-1.5">
        <div
          ...
          :key="item.id"
          class="flex items-center gap-3 rounded-2xl bg-card ring-1 ring-border/60 dark:ring-0 px-4 py-3.5 transition active:scale-[0.99]"
          :class="editing ? 'cursor-pointer' : 'cursor-pointer'"
      ...
      <EmptyState
        ...
        :icon="Star"
        title="관심 종목이 비어 있어요"
        description="위 검색창에서 종목을 찾아 관심에 담아보세요. 가격이 한눈에 들어와요."
      />
